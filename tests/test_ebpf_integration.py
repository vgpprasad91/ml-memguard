...
Covers:
  ...
  MemguardWatcher (cloud) — BPF signal merge
  -----------------------------------------------
  17. BPF signals in _tick() payload when session available
  18. No BPF signals in _tick() payload when session=None
"""
...
# ===========================================================================
# 17–18. MemguardWatcher (cloud) — BPF signal merge in _tick()
# ===========================================================================

class TestMemguardWatcherEBPF:

    def _make_watcher(self, ebpf_session=None) -> Any:
        """Import MemguardWatcher and build a minimal instance."""
        import sys
        _CLOUD_PATH = "/Users/guruprasadvenkataraghavan/Documents/Dropbox/Projects/Side_Projects/memguard-cloud"
        if _CLOUD_PATH not in sys.path:
            sys.path.insert(0, _CLOUD_PATH)
        from memguard_cloud.watch import MemguardWatcher  # noqa: PLC0415
        return MemguardWatcher(
            engine=None,
            api_key="test-key",
            ebpf_session=ebpf_session,
        )

    def test_bpf_signals_in_tick_payload_when_session_active(self):
        """BPF signals page_fault_rate and memory_pressure_level appear in payload."""
        session = _make_fake_session(
            available=True,
            page_fault_rate=33.3,
            memory_pressure_bytes=64 * 1024 * 1024,
        )
        watcher = self._make_watcher(ebpf_session=session)
        ...
        with patch.object(watcher, "_post_predict", side_effect=_fake_post):
            watcher._tick()

        assert "page_fault_rate" in captured_payload
        assert captured_payload["page_fault_rate"] == pytest.approx(33.3)
        assert "memory_pressure_level" in captured_payload
        # 64 MiB → 64.0 MB
        assert captured_payload["memory_pressure_level"] == pytest.approx(64.0)

    def test_no_bpf_signals_when_session_none(self):
        """When ebpf_session=None, BPF keys are absent from the predict payload."""
        watcher = self._make_watcher(ebpf_session=None)
        ...
        assert "page_fault_rate" not in captured_payload
        assert "memory_pressure_level" not in captured_payload
